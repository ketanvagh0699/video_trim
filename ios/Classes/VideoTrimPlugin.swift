...
public class VideoTrimPlugin: NSObject, FlutterPlugin {
  ...
  private func trimVideo(inputURL: URL, startTime: CMTime, endTime: CMTime) throws -> String {
    ...
    guard let videoTrack = asset.tracks(withMediaType: .video).first else {
      throw NSError(domain: "VideoTrimPluginError", code: 500, userInfo: [NSLocalizedDescriptionKey: "No video track found"])
    }
    ...
    guard let exporter = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetHighestQuality) else {
      throw NSError(domain: "VideoTrimPluginError", code: 500, userInfo: [NSLocalizedDescriptionKey: "Failed to create export session"])
    }
    ...
    if exporter.status == .completed {
      ...
      throw NSError(domain: "VideoTrimPluginError", code: 500, userInfo: [NSLocalizedDescriptionKey: "Export failed: \(exporter.error?.localizedDescription ?? "Unknown error")"])
    }
  }
